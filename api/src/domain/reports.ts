...
export interface MonthlyRow {
  ...
  roi: string | null;
  /** Расходы на содержание бизнеса за тот же календарный месяц (business_expenses, без категории
   * CARD — «На карту» не расход) плюс аренда, не связаны
  ...
  cashless: string;
  /** Сумма записей категории CARD за месяц: наличные, которые уже физически собраны и переданы
   * владельцу отдельным переводом. Это не расход: в expensesTotal и netProfit не входит.
   * Второе слагаемое reachedOwner. */
  paidToOwner: string;
...
export async function monthlyReport(
  ...
  // владельцу отдельным переводом: он получает эти деньги себе (владелец уточнил 19.09.2026,
  // DECISION-080). Поэтому выше она исключена из expensesTotal и не уменьшает netProfit. Здесь она
  // нужна как один из двух каналов, которыми деньги реально доходят до владельца, — второй канал
  // ниже, безналичная выручка с терминалов.
