...
#
# Что скрипт делает для каждой из 19 строк ниже: рендерит картинку строки таблицы (уже
# подготовлена заранее, лежит в .import-fedor-images/), загружает её как фото обслуживания и
# создаёт обслуживание технику Фёдору (technicianId по DECISION-055, так как заводит
# администратор).
#
# Что сюда НЕ входит — и почему:
#   - № 111 (07.09 и 08.09) и № 113 (10.09) — владелец сообщил, что их настоящая установка и
#     база счётчика были заведены неверно (артефакт реорганизации каталога 06.09); эти три
#     строки, плюс восстановленные по словам владельца промежуточные показания, перенесены в
#     отдельный scripts/fix-111-113-and-import-fedor.sh, который сначала правит базу, а потом
#     создаёт обслуживания — иначе рост счётчика получился бы физически невозможным
#   - № 18 за 10.09 (4531) — меньше уже прочитанного 14531 за 07.09, счётчик не может идти назад;
#     владелец подтвердил, что после 07.09 обслуживаний по этому аппарату не было
#   - № 19 за 10.09 (20210) — владелец подтвердил, что 07.09 было последним обслуживанием
#   - № 15 за 11.09 (372) — владелец попросил не трогать аппарат 15 отдельно
#   - № 9 за 11.09 — исправление показания (21905→20198 по подтверждению владельца) тоже вынесено
#     в scripts/fix-111-113-and-import-fedor.sh
